/**
 * Glifos oficiais das redes, em SVG.
 *
 * Existiam antes como texto dentro de um círculo colorido ("in", "IG", um
 * caractere unicode para o X). Achado do teste de jornada de 21/08: parece
 * improviso, e numa tela que pede autorização para publicar em nome da pessoa,
 * parecer improviso custa confiança.
 *
 * Cada marca vem com a cor oficial de fundo e o desenho em branco, que é o uso
 * permitido pelas diretrizes de marca das quatro plataformas. Os caminhos são
 * os glifos das próprias marcas, sem deformação e sem recorte.
 */

type Props = { className?: string };

const BASE = "w-9 h-9 rounded-lg flex items-center justify-center shrink-0";

export function LogoLinkedIn({ className = "" }: Props) {
  return (
    <span className={`${BASE} ${className}`} style={{ background: "#0A66C2" }}>
      <svg viewBox="0 0 24 24" width="20" height="20" fill="#fff" aria-hidden>
        <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.03-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05a3.74 3.74 0 0 1 3.37-1.85c3.6 0 4.27 2.37 4.27 5.46v6.28zM5.34 7.43a2.06 2.06 0 1 1 0-4.13 2.06 2.06 0 0 1 0 4.13zM7.12 20.45H3.55V9h3.57v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.72v20.56C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.72V1.72C24 .77 23.2 0 22.22 0z" />
      </svg>
    </span>
  );
}

export function LogoX({ className = "" }: Props) {
  return (
    <span className={`${BASE} ${className}`} style={{ background: "#000" }}>
      <svg viewBox="0 0 24 24" width="18" height="18" fill="#fff" aria-hidden>
        <path d="M18.9 1.15h3.68l-8.04 9.19L24 22.85h-7.41l-5.8-7.58-6.64 7.58H.46l8.6-9.83L0 1.15h7.59l5.24 6.93 6.07-6.93zm-1.29 19.5h2.04L6.49 3.24H4.3l13.31 17.41z" />
      </svg>
    </span>
  );
}

export function LogoInstagram({ className = "" }: Props) {
  return (
    <span
      className={`${BASE} ${className}`}
      style={{
        background:
          "radial-gradient(circle at 30% 107%, #fdf497 0%, #fdf497 5%, #fd5949 45%, #d6249f 60%, #285AEB 90%)",
      }}
    >
      <svg viewBox="0 0 24 24" width="20" height="20" fill="#fff" aria-hidden>
        <path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41-.56-.22-.96-.48-1.38-.9-.42-.42-.68-.82-.9-1.38-.16-.42-.36-1.06-.41-2.23C2.17 15.58 2.16 15.2 2.16 12s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16zM12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.3-1.46.72-2.12 1.39C1.35 2.68.93 3.35.63 4.14.33 4.9.13 5.78.07 7.05.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.3.79.72 1.46 1.39 2.12.66.67 1.33 1.09 2.12 1.39.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56.79-.3 1.46-.72 2.12-1.39.67-.66 1.09-1.33 1.39-2.12.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91-.3-.79-.72-1.46-1.39-2.12C21.32 1.35 20.65.93 19.86.63c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0zm0 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm7.85-10.4a1.44 1.44 0 1 1-2.88 0 1.44 1.44 0 0 1 2.88 0z" />
      </svg>
    </span>
  );
}

export function LogoFacebook({ className = "" }: Props) {
  return (
    <span className={`${BASE} ${className}`} style={{ background: "#1877F2" }}>
      <svg viewBox="0 0 24 24" width="20" height="20" fill="#fff" aria-hidden>
        <path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.09 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.69.24 2.69.24v2.96h-1.51c-1.49 0-1.96.93-1.96 1.89v2.26h3.33l-.53 3.49h-2.8V24C19.61 23.09 24 18.1 24 12.07z" />
      </svg>
    </span>
  );
}

export function LogoYouTube({ className = "" }: Props) {
  return (
    <span className={`${BASE} ${className}`} style={{ background: "#FF0000" }}>
      <svg viewBox="0 0 24 24" width="20" height="20" fill="#fff" aria-hidden>
        <path d="M23.5 6.19a3.02 3.02 0 0 0-2.12-2.14C19.5 3.55 12 3.55 12 3.55s-7.5 0-9.38.5A3.02 3.02 0 0 0 .5 6.19C0 8.08 0 12 0 12s0 3.92.5 5.81a3.02 3.02 0 0 0 2.12 2.14c1.88.5 9.38.5 9.38.5s7.5 0 9.38-.5a3.02 3.02 0 0 0 2.12-2.14C24 15.92 24 12 24 12s0-3.92-.5-5.81zM9.55 15.57V8.43L15.82 12l-6.27 3.57z" />
      </svg>
    </span>
  );
}

export const LOGO_POR_REDE = {
  linkedin: LogoLinkedIn,
  twitter: LogoX,
  instagram: LogoInstagram,
  facebook: LogoFacebook,
  youtube: LogoYouTube,
} as const;

export type RedeComLogo = keyof typeof LOGO_POR_REDE;
